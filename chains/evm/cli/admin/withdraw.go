...
func BindWithdrawCmdFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(&TokenID, "token-id", "", "Token ID to withdraw, use only if ERC721 token is withdrawn. If both amount and token-id are set an error will occur")
