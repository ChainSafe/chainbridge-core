...
func (s *IntegrationTestSuite) SetupTest() {
	...
	bridgeAddr, erc20Addr, erc20HandlerAddr, err := local.PrepareLocalEnv(ethClient, s.fabric1, uint8(1), big.NewInt(1), s.adminKey.CommonAddress(), big.NewInt(0))
	if err != nil {
		panic(err)
	}
	s.client = ethClient
	...
	_, _, _, err = local.PrepareLocalEnv(ethClient2, s.fabric2, uint8(2), big.NewInt(1), s.adminKey.CommonAddress(), big.NewInt(0))
	if err != nil {
		panic(err)
	}
}
