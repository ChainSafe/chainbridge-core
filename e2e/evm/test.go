...
func (s *IntegrationTestSuite) SetupTest() {
	...
	bridgeAddr, erc20Addr, erc20HandlerAddr, err := PrepareEVME2EEnv(ethClient, s.fabric1, 1, big.NewInt(2), s.adminKey.CommonAddress(), big.NewInt(0))
	if err != nil {
		panic(err)
	}
	s.client = ethClient
	...
	_, _, _, err = PrepareEVME2EEnv(ethClient2, s.fabric2, 2, big.NewInt(2), s.adminKey.CommonAddress(), big.NewInt(0))
	if err != nil {
		panic(err)
	}
}
