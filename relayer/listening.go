...
type Handler func(destID uint64, nonce uint64) (*Messager, error)
...
func getDepositEventsForBlock(l IListener, latestBlock *big.Int) error {
	log.Info().Msgf("geting deposit events for block %s", latestBlock.String())
	...
	for _, eventLog := range logs {
		...
		destId := eventLog.Topics[1].Big().Uint64()
